# Gene Expression Analysis (Cancer vs Normal)

## Overview
This project explores gene expression differences between normal and cancer conditions using a simplified dataset, inspired by publicly available gene expression studies.

## Dataset
- Inspired by gene expression datasets from GEO (I randomly chose some dataset, such as GSE75970 for reference)
- Focus on well-known cancer-related genes:
  - Oncogenes: MYC, EGFR, KRAS
  - Tumor suppressors: TP53, PTEN

## Methods
- Data processing using Python (pandas)
- Data transformation using pivot tables
- Visualization using matplotlib

## Results
- MYC, EGFR, and KRAS are upregulated in cancer
- TP53 and PTEN are downregulated
- These patterns align with known biological mechanisms in cancer development

## Key Insight
Cancer progression is often characterized by the activation of oncogenes and the suppression of tumor suppressor genes, as reflected in the observed expression patterns.

## Skills Demonstrated
- Basic data analysis (Python, pandas)
- Data visualization
- Biological interpretation of gene expression data
